...
public class EventEdit extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
  {
    ...
    request.setAttribute("type", event.getType());

    RequestDispatcher view;
    request.setAttribute("event", event);
    System.out.println("dochodzi do view");
    if(event.getType()==0){
    view= request.getRequestDispatcher("WEB-INF/views/edit/EventEdit.jsp");}
    else
    {
      view= request.getRequestDispatcher("WEB-INF/views/edit/TaskEdit.jsp");
    }
    try{
